...
const Page = () => {
  ...
  const { data: customers, isValidating: tableIsValidating, error } = useSWR(key, fetcher);
  ...
  async function fetcher(url) {
    try {
      ...
      if (payload && payload.status == true) {
        const customers = payload.data.filter(user => user.user_type === 'customer');
        return customers;
  ...
  return (
    <div className="bg-white shadow-xl rounded-md p-8">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-semibold tracking-wide uppercase">Customers</h1>
        <Button
      ...
      <Card className="">
        <CardContent>
          ...
          {/* <div className="flex justify-between items-center mt-4">
            <span className="text-sm font-normal">Showing 1-10 from 100 data</span>
            <div className="flex space-x-1">
              <Button variant="ghost">
                <ChevronLeftIcon className="text-gray-500" />
              </Button>
              <Button className="border-blue-500 text-blue-500" variant="outline">
                1
              </Button>
