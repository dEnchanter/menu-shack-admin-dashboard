...
const Page = () => {
  ...
  const { data: drivers, isValidating: tableIsValidating, error } = useSWR(key, fetcher);
  ...
  async function fetcher(url) {
    try {
      ...
      if (payload && payload.status == true) {
        const drivers = payload.data.filter(user => user.user_type === 'driver');
        return drivers;
  ...
  return (
    <div className="bg-white shadow-xl rounded-md p-8">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-semibold tracking-wide uppercase">Drivers</h1>
        <Button
